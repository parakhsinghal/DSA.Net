...
{
    /// <summary>
    /// MinHeap is a class derived from the BinaryHeap class. It focuses on
    /// implementing a heap with the minimum heap property thereby, arranging 
    /// the elements in such a way that the parents are always less than the 
    /// children.
    /// </summary>
    /// <typeparam name="T">The data type of the MinHeap class. E.g. int, char or a user defined type.</typeparam>
    public class MinHeap<T> : BinaryHeap<T>
    {
        ...
        protected override T Maximum
        {
            get
            {
                ...
                {
                    throw new InvalidOperationException(Err.BinaryHeap_Max_EmptyHeap);
                }
            }
        }
        ...
        protected override void HeapifyUpIterative(int indexOfElement)
        {
            ...
            try
            {
                for (int i = indexOfElement; i >= 0; i--)
                {
                    ...
                    {
                        case < 0:       // The element is less than parent and thus needs to be pushed up
